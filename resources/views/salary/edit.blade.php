...
@section('content')
    ...
    <div class="salary-edit-container">
        ...
        <form id="salary-edit-form" method="POST" action="{{ route('salary.update', $salary['id']) }}">
            @csrf
            @method('PUT')
            <input type="hidden" name="employee_id" value="{{ $salary['employee_id'] ?? $salary['employee'] ?? '' }}">

            <!-- Employee Selector -->
            <div class="employee-selector">
                <label>Employee:</label>
                <input type="text"
                    value="{{ $salary['employee'] ?? $salary['employee_id'] ?? '' }} - {{ $salary['employee_name'] ?? 'Unknown' }}"
                    readonly>
            ...
            <div class="form-row">
                ...
                <input type="number" step="0.01" name="basic_salary"
                    value="{{ $salary['basic_salary'] ?? $salary['base_salary'] ?? '' }}">
            </div>
            ...
            <div class="form-row">
                ...
                <input type="number" name="weekends"
                    value="{{ $salary['weekends'] ?? $salary['working_days_including_weekends'] ?? '' }}">
            </div>
            ...
            <div class="form-row">
                ...
                <input type="number" name="attendance_days"
                    value="{{ $salary['attendance_days'] ?? $salary['attended_days'] ?? '' }}">
            </div>
            ...
            <div class="form-row">
                <label>OT rate:</label>
                <input type="number" step="0.01" name="ot_rate" value="{{ $salary['ot_rate'] ?? '' }}">
            </div>

            <div class="form-row">
                <label>Holiday allowance:</label>
                <input type="number" step="0.01" name="hd_allowance" value="{{ $salary['hd_allowance'] ?? '' }}">
            </div>
            ...
            <div class="form-row">
                ...
                <input type="number" step="0.01" name="required_attendance_percent"
                    value="{{ $salary['required_attendance_percent'] ?? '' }}">
            ...
            <div class="form-row">
                ...
                <input type="hidden" name="absent_fine_mode" value="per_day">
                <input type="number" step="0.01" name="absent_fine" value="{{ $salary['absent_fine_per_day'] ?? $salary['absent_fine'] ?? '' }}">
                <div class="help-text">
                    Per-day absent fine amount.
                    Absent days: {{ $salary['absent_days'] ?? 0 }}.
                    Current total: {{ number_format((float)($salary['absent_fine_total'] ?? $salary['absent_fine'] ?? 0), 2) }}
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <input type="number" step="0.01" name="tds_percent"
                    value="{{ $salary['tds_percent'] ?? $salary['tax_payment'] ?? '' }}">
            </div>

            <div class="form-row">
                <label>Stamp:</label>
                <input type="number" step="0.01" name="stamp" value="{{ $salary['stamp'] ?? '' }}">
            </div>
            ...
            <div class="button-group">
                ...
                <button type="button" class="btn btn-secondary" onclick="continueEditing()">Save and continue
                    editing</button>
    ...
    <script>
        function continueEditing() {
            const form = document.getElementById('salary-edit-form');
            if (!form) return;
